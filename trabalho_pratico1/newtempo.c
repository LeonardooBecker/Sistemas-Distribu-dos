...
int main(int argc, char *argv[])
{
   ...
   int processoFalho = 3;
   schedule(fault, 0.0, processoFalho);
   ...
   while (1)
   {
      ...
      case test:
         ...
         for (int s = 1; s <= log2(N); s++)
         {
            ...
            for (int j = 0; j < nodes->size; j++)
            {
               ...
               if (status(processo[tokenTeste].id) == 0)
               {
                  ...
                  if ((processo[token].STATE[processoFalho] == -1) || (processo[token].STATE[processoFalho] == 0))
                     latencia[token]++;
         ...
         for (int i = 0; i < N; i++)
         {
            if ((processo[i].STATE[processoFalho] == -1) || (processo[i].STATE[processoFalho] == 0))
            {
               finalizaLoop = 0;
            }
   ...
   for (int i = 0; i < N; i++)
   {
      if (latencia[i] > latPiorCaso)
      {
         latPiorCaso = latencia[i];
      }
      // printf("Latencia do processo %d: %d\n", i, latencia[i]);
   }
   printf("Latencia do pior caso:%d\n", latPiorCaso);
